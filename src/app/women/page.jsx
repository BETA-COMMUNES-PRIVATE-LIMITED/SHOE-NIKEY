...
import { categories, sizes, colorOptions, sortOptions } from '@/data/womenProducts';
...
export default function WomenPage() {
  ...
  const allProducts = useMemo(
    () => [...applyProductOverrides(productsData), ...customProducts.filter((p) => p.section === 'women')],
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: 'var(--bg-primary)' }}>
      ...
      <main className="flex-1 w-full max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 py-6 md:py-8">
        <Breadcrumb title="WOMEN" section="Women" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product) => (
              <ProductCard key={product.id} product={product} section="women" />
            ))
          ) : (
