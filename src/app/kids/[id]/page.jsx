...
export default function ProductDetailPage() {
  ...
  const inWishlist = product ? isInWishlist(product.id, 'kids') : false;
      ...
      <div className="min-h-screen flex flex-col" style={{ backgroundColor: 'var(--bg-primary)' }}>
        ...
        <main className="flex-1 flex items-center justify-center px-4">
          <div className="text-center">
            ...
            <Link href="/kids" className="text-sm underline" style={{ color: 'var(--accent-lime)' }}>Back to Kids</Link>
          </div>
        </main>
      </div>
    );
  ...
  const handleToggleWishlist = () => {
    toggleWishlist({ ...product, section: 'kids' });
  };
          ...
          <div className="flex flex-col-reverse md:flex-row gap-3 md:gap-4">
                ...
                {currentImages.map((img, idx) => (
                  ...
                  >
                    <div className="relative w-full h-full">
                      <Image src={img} alt={`${product.name} ${idx + 1}`} fill className="object-contain p-1" style={idx === 2 ? { transform: 'scale(1.25) translateX(-4%)' } : undefined} />
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="relative w-full aspect-square rounded-2xl overflow-hidden" style={{ backgroundColor: 'var(--bg-card)' }}>
              ...
              <Image
                ...
                style={{ transform: 'scale(1.15) translateX(-3%)' }}
                priority
              />
              {/* Navigation arrows */}
              {currentImages.length > 1 && (
          ...
          <div className="flex flex-col gap-4 md:gap-5">
            ...
            {product.isComingSoon ? (
              <div className="flex items-center gap-3 py-4">
                <span className="text-sm font-bold uppercase px-4 py-2 rounded-md" style={{ backgroundColor: 'var(--accent-red)', color: '#FFFFFF' }}>
                  Coming Soon
                </span>
                <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Stay tuned for updates!</p>
              </div>
            ) : (
              <>
                {/* Color */}
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-xs font-bold" style={{ color: 'var(--text-primary)' }}>
                      SIZE: <span style={{ color: 'var(--text-muted)' }}>{selectedSize ? selectedSize : 'Select'}</span>
                    </p>
