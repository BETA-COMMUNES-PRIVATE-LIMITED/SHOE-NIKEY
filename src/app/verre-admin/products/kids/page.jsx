...
export default function AdminKidsProductsPage() {
  ...
  // Deep-link: /verre-admin/products/kids?edit=ID opens the editor for that product
  useEffect(() => {
    const id = Number(new URLSearchParams(window.location.search).get('edit'));
    if (!id) return;
    const all = [...applyProductOverrides(productsDataKids.map((p) => ({ ...p, section: 'kids', isBuiltIn: true }))), ...customProducts.filter((p) => p.section === 'kids')];
  ...
  const builtIn = applyProductOverrides(productsDataKids.map((p) => ({ ...p, section: 'kids', isBuiltIn: true })));
