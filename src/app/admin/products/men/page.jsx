  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-lg font-extrabold" style={{ color: 'var(--text-primary)' }}>Men&apos;s Products</h1>
          ...
          <div className="w-full max-w-sm rounded-2xl p-6 animate-dropdown" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)}
                className="flex-1 h-10 rounded-full text-xs font-semibold transition-all active:scale-95"
                style={{ border: '1px solid var(--border-color)', color: 'var(--text-muted)' }}
              >Cancel</button>
              <button onClick={() => handleDelete(deleteConfirm.id)}
                className="flex-1 h-10 rounded-full text-xs font-bold transition-all active:scale-95"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'Total', value: totalProducts, color: '#60a5fa' },
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 p-1 rounded-xl" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
            <button onClick={() => setViewMode('grid')}
              ...
              style={{ backgroundColor: viewMode === 'grid' ? 'var(--accent-lime)' : 'transparent', color: viewMode === 'grid' ? '#000' : 'var(--text-muted)' }}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
            ...
            <button onClick={() => setViewMode('list')}
              className="p-2 rounded-lg transition-all"
              style={{ backgroundColor: viewMode === 'list' ? 'var(--accent-lime)' : 'transparent', color: viewMode === 'list' ? '#000' : 'var(--text-muted)' }}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-hide">
          {categories.map((c) => (
            <button key={c} onClick={() => setActiveCategory(c)}
              ...
              style={{
                backgroundColor: activeCategory === c ? '#60a5fa' : 'var(--bg-surface)',
          ...
          {filtered.map((product) => (
            ...
            >
              <div className="relative w-full aspect-square" style={{ backgroundColor: 'var(--bg-surface)' }}>
                ...
                {product.isNew && (
                  <span className="absolute top-2 left-2 text-[9px] font-bold px-2 py-0.5 rounded" style={{ backgroundColor: 'var(--accent-lime)', color: '#000' }}>NEW</span>
                )}
                {product.isComingSoon && (
                  <span className="absolute top-2 left-2 text-[9px] font-bold px-2 py-0.5 rounded" style={{ backgroundColor: 'var(--accent-red)', color: '#fff' }}>SOON</span>
                )}
                {/* Hover Actions */}
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-2">
                  ...
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" style={{ color: '#fff' }}>
                  ...
                  {!product.isBuiltIn && (
                    <button onClick={() => setDeleteConfirm(product)}
                      className="w-10 h-10 rounded-full flex items-center justify-center transition-all hover:scale-110"
                      style={{ backgroundColor: 'rgba(239, 68, 68, 0.3)', backdropFilter: 'blur(4px)' }}
                    >
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-2 mt-3">
                  <span className="text-[9px] font-bold px-2 py-0.5 rounded" style={{ backgroundColor: 'var(--bg-surface)', color: 'var(--text-muted)', border: '1px solid var(--border-color)' }}>
                    {product.category}
                  </span>
                  {product.isBuiltIn ? (
                    <span className="text-[9px] font-bold px-2 py-0.5 rounded" style={{ backgroundColor: 'rgba(96, 165, 250, 0.1)', color: '#60a5fa', border: '1px solid rgba(96, 165, 250, 0.2)' }}>Built-in</span>
                  ) : (
                    <span className="text-[9px] font-bold px-2 py-0.5 rounded" style={{ backgroundColor: 'rgba(166, 255, 0, 0.1)', color: 'var(--accent-lime)', border: '1px solid rgba(166, 255, 0, 0.2)' }}>Custom</span>
                  )}
                </div>
              </div>
            </div>
          ...
          {filtered.map((product) => (
            ...
            >
              ...
              <div className="col-span-2 md:col-span-1 hidden md:block">
                {product.isComingSoon ? (
                  <span className="text-[9px] font-bold px-2 py-0.5 rounded" style={{ backgroundColor: 'rgba(251, 191, 36, 0.1)', color: '#fbbf24' }}>Soon</span>
                ) : product.isNew ? (
                  <span className="text-[9px] font-bold px-2 py-0.5 rounded" style={{ backgroundColor: 'rgba(166, 255, 0, 0.1)', color: 'var(--accent-lime)' }}>New</span>
                ) : (
                  <span className="text-[9px] font-bold px-2 py-0.5 rounded" style={{ backgroundColor: 'rgba(34, 197, 94, 0.1)', color: '#22c55e' }}>Active</span>
                )}
              </div>
              <div className="col-span-12 md:col-span-3 flex items-center justify-end gap-2">
                <button onClick={() => handleEdit(product)}
                  className="text-[10px] font-bold px-3 py-1.5 rounded-lg flex items-center gap-1 transition-all hover:bg-blue-500/10"
                ...
                {!product.isBuiltIn && (
                  <button onClick={() => setDeleteConfirm(product)}
                    className="text-[10px] font-bold px-3 py-1.5 rounded-lg flex items-center gap-1 transition-all hover:bg-red-500/10"
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16 rounded-2xl" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
          <svg className="w-12 h-12 mx-auto mb-3" fill="none" stroke="currentColor" strokeWidth="1" viewBox="0 0 24 24" style={{ color: 'var(--text-muted)' }}>
            <circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" />
          </svg>
